...
class PushAnimator: NSObject, UIViewControllerAnimatedTransitioning {
    ...
    func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
        ...
        UIView.animateKeyframes(withDuration: animationDuration, delay: 0, options: .calculationModePaced, animations: {
            let povorot = CGAffineTransform(rotationAngle: 90)
            let smeshenie = CGAffineTransform(translationX: -destination.view.bounds.width, y: -destination.view.bounds.height)
            
            destination.view.transform = povorot.concatenating(smeshenie)
            
//            UIView.addKeyframe(withRelativeStartTime: 0, relativeDuration: 0.4) {
//                let t: CGAffineTransform = CGAffineTransform(rotationAngle: 0 )
...
class PopAnimator: NSObject, UIViewControllerAnimatedTransitioning {
    ...
    func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
        ...
        UIView.animateKeyframes(withDuration: animationDuration, delay: 0, options: .calculationModePaced, animations: {
            ...
            UIView.addKeyframe(withRelativeStartTime: 0, relativeDuration: 0.4) {
                let t: CGAffineTransform = CGAffineTransform(rotationAngle: 90)
                let smeshenie = CGAffineTransform(translationX: -source.view.bounds.width, y: -source.view.bounds.height)
                
                source.view.transform = t.concatenating(smeshenie)
            }
